...
export default function ImportadorLogsTunagem({
  ...
  // Helper para identificar a oficina de um log pelo técnico
  const identificarOficina = (logOficinaNome, tecId, dataLog) => {
    ...
    // 3. Caso não encontre vínculo, usa o fallback selecionado
  ...
  const executarImportacao = async () => {
    ...
    if (!confirm(\`🚀 Deseja iniciar a importação de \${logsIdentificados.length} logs de tunagem para o banco de dados?\\n\\nO sistema associou cada log à sua respectiva oficina com base no cadastro dos mecânicos.\\nRegistros com o mesmo UUID serão atualizados sem duplicar.\`)) {
      return;
    }
    ...
    try {
      ...
      alert(\`✅ Importação concluída com sucesso!\\n\\n\${inseridosOuAtualizados} logs salvos/atualizados e distribuídos por oficina no banco de dados.\`);
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
          <div>
            ...
            <p style={{ margin: "6px 0 0", fontSize: "13px", color: theme.subtext }}>
              O importador analisa o <strong>ID de cada técnico</strong> para identificar automaticamente se o serviço pertence à <strong>RED'S, BENNY'S, EAST</strong> ou outras oficinas.
            </p>
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <label style={{ fontSize: "12px", color: theme.subtext, fontWeight: "700" }}>Oficina Fallback (se técnico não vinculado):</label>
            <input
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: "12px", color: theme.subtext, maxWidth: "340px" }}>
              Suporta exportação do canal do Discord com 1.000+ mensagens de todas as oficinas.
            </p>
            <button
              type="button"
              style={{
      ...
      {estatisticas && (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div style={{ background: theme.card, border: "1px solid #10b981", borderRadius: "16px", padding: "24px" }}>
            ...
            {/* Distribuição Identificada por Oficina */}
            <div style={{ background: "rgba(0,0,0,0.25)", border: \`1px solid \${theme.border}\`, borderRadius: "12px", padding: "16px" }}>
...
fs.writeFileSync("app/components/ImportadorLogsTunagem.jsx", componentCode, "utf8");
console.log("Updated app/components/ImportadorLogsTunagem.jsx successfully!");
  ...
  const [oficinaPadrao, setOficinaPadrao] = useState("RED'S TUNERSHOP");
  ...
  const processarConteudo = (conteudoBruto, nomeOrigem = "Entrada manual") => {
    ...
    try {
      ...
      for (const msgTexto of mensagensTexto) {
        ...
        for (const log of logsDaMensagem) {
          if (log.uuid) {
            // Se o log tiver oficina como '?', define a padrão se fornecida
            if (!log.oficina_nome || log.oficina_nome === "?") {
              log.oficina_nome = oficinaPadrao || "RED'S TUNERSHOP";
            }
            if (!log.baia_nome || log.baia_nome === "?") {
              log.baia_nome = "Geral";
            }
      ...
      // 3. Calcula Estatísticas do Lote
      const tecnicosSet = new Set();
      const veiculosSet = new Set();
      let totalValorPago = 0;
      let dataMaisAntiga = listaLogsUnicos[0].data;
      ...
      for (const log of listaLogsUnicos) {
        if (log.tecnico_nome) tecnicosSet.add(`${log.tecnico_nome} (ID: ${log.tecnico_id})`);
        if (log.veiculo_nome) veiculosSet.add(log.veiculo_nome);
        totalValorPago += Number(log.valor_pago || 0);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: "16px", padding: "24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
          <div>
            <h3 style={{ margin: 0, fontSize: "18px", fontWeight: "900", color: "#10b981", display: "flex", alignItems: "center", gap: "10px" }}>
              📥 Importador Inteligente de Logs do Discord (.JSON / Texto)
            </h3>
            <p style={{ margin: "6px 0 0", fontSize: "13px", color: theme.subtext }}>
              Envie o arquivo <code style={{ background: "rgba(255,255,255,0.08)", padding: "2px 6px", borderRadius: "4px" }}>.json</code> exportado do canal do Discord para alimentar o banco de dados da oficina automaticamente.
            </p>
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <label style={{ fontSize: "12px", color: theme.subtext, fontWeight: "700" }}>Oficina Padrão:</label>
            <input
              value={oficinaPadrao}
              onChange={(e) => setOficinaPadrao(e.target.value)}
              placeholder="RED'S TUNERSHOP"
              style={{
                background: theme.card2,
                border: `1px solid ${theme.border}`,
      ...
      {estatisticas && (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: "16px", padding: "20px", overflowX: "auto" }}>
            <h4 style={{ margin: "0 0 14px 0", fontSize: "15px", fontWeight: "800", color: "#fff", display: "flex", alignItems: "center", gap: "8px" }}>
              👀 Amostra dos Logs Prontos para Ingestão (Mostrando primeiros 10 de ${logsIdentificados.length})
            </h4>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", textAlign: "left" }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${theme.border}`, color: theme.subtext }}>
                  ...
                  <th style={{ padding: "10px 12px" }}>Veículo (Placa)</th>
                  <th style={{ padding: "10px 12px", textAlign: "right" }}>Valor In-Game</th>
                  <th style={{ padding: "10px 12px" }}>UUID</th>
                </tr>
              </thead>
              <tbody>
                {logsIdentificados.slice(0, 10).map((l) => (
                  <tr key={l.uuid} style={{ borderBottom: `1px solid ${theme.border}` }}>
                    ...
                    <td style={{ padding: "10px 12px", fontFamily: "monospace", fontSize: "10px", color: theme.subtext }}>
                      {l.uuid.slice(0, 13)}...
                    </td>
                  </tr>
                ))}
...
fs.writeFileSync("app/components/ImportadorLogsTunagem.jsx", componentCode, "utf8");
console.log("File app/components/ImportadorLogsTunagem.jsx created successfully!");
